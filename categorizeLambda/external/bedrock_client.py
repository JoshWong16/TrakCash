...
class BedrockClient:
    ...
    def categorize_transactions(self, transactions: List[Transaction], categories_text: str) -> List[Transaction]:
        """Send transactions to Bedrock for categorization"""
        try:
            print(f"Categorizing {len(transactions)} transactions with Bedrock")
            
            prompt = self._create_prompt(transactions, categories_text)
            ...
            return self._parse_response(text_content, transactions)
            
        except Exception as e:
            print(f"Error categorizing transactions with Bedrock: {str(e)}")
            raise 
          
             
    def create_prompt(self, transactions: List[Transaction], categories_text: str) -> str:
        """Create the prompt for Bedrock"""
        transactions_data = [t.to_bedrock_dict() for t in transactions]
        transactions_str = json.dumps(transactions_data, indent=2)
    ...
    def _parse_response(self, text_content: str, transactions: List[Transaction]) -> List[Transaction]:
        """Parse Bedrock response and update transactions"""
        ...
        if json_start >= 0 and json_end > json_start:
            ...
            transaction_map = {t.transaction_id: t for t in transactions}
